...
### 7. Camera privacy (already live on the Pi — mostly no host change)
The device now blurs every person in every `camera.front` frame and re-encodes
it before it leaves pDAL. The frame is still a valid JPEG, same dimensions,
`content_type: image/jpeg`, and the `PDALSTR1` framing is unchanged (the
`payload_size` prefix already reflects the emitted size). The viewer's decode /
display path needs **no change** — it just shows blurred people.

- [ ] Handle a `503` on a camera request: if the blur stage fails or is
      unavailable, pDAL returns `503 PDAL_BACKEND_UNAVAILABLE` and **no frame**.
      Show "camera unavailable" (fail closed), not a broken image.
- [ ] Bandwidth panel: blurred+re-encoded frames are ~0.75× the stored size at
      the default quality (tunable). If the UI shows "network bytes vs onboard
      bytes", label it so the ratio isn't read as raw-vs-raw compression.
- [ ] Optional: a small "frames anonymized on-device" badge near the camera view.
- [ ] `smoke_test.py`: the JPEG signature (`FF D8 FF`) is unchanged so that
      assertion still holds; do **not** assert the camera payload size equals the
      stored size.

### 8. Docs (host `README` / host docs)
- [ ] Document the login step in the run instructions: open `:8088`, pick a role,
      enter its password (`fleet-demo` etc.).
- [ ] State that camera frames arrive already blurred; the host still does all
      rendering. `/api/health` `decode_location: host` refers to render decode —
      the Pi's internal decode/re-encode for blur is the documented exception.
...
- [ ] Camera view shows people blurred; a camera `503` shows "unavailable", not a
      broken image.
